...
export default function VendorEarningsScreen() {
  ...
  // const createOrUpdateBankDetails = useCreateOrUpdateVendorBankDetails();
  const cancelCashout = useCancelCashout();

  // Local state
  const [cashoutModalVisible, setCashoutModalVisible] = useState(false);
  const [cashoutAmount, setCashoutAmount] = useState('');
  const [activeEarningsFilter, setActiveEarningsFilter] = useState<
    'today' | 'week' | 'month'
  >('today');
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert(
                'Success',
                `Cashout request of ₹${amount.toLocaleString()} submitted!\n\nIt will be transferred within 2 business days.`
              );
              setCashoutAmount('');
  ...
  const handleEditBankAccount = () => {
    // Navigate to bank account edit screen or show modal
    router.push("/vendor/profile/documents/bank/update")
  };

  const handleSettlementHistory = () => {
    // Navigate to full settlement history screen
  ...
  const getEarningsAmount = () => {
    ...
    switch (activeEarningsFilter) {
      case 'today':
        return wallet.data.earnings_today;
      case 'week':
        return wallet.data.earnings_this_week;
      case 'month':
        return wallet.data.earnings_this_month;
      default:
        return 0;
    }
  };
  ...
  const handleCancelCashout = (cashoutId: string) => {
    Alert.alert(
      ...
      'Are you sure you want to cancel this cashout request? The amount will be returned to your available balance.',
      [
        { text: 'No', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await cancelCashout.mutateAsync(cashoutId);
              Alert.alert('Success', 'Cashout request cancelled successfully');
            } catch (error) {
              console.error('Cancel cashout error:', error);
              Alert.alert('Error', 'Failed to cancel cashout request');
            }
          },
        },
      ]
      ...
      >
        ...
        <View className="bg-white mx-4 mt-4 rounded-2xl p-4 shadow-sm border border-gray-100">
          ...
          <Text className="text-xs text-gray-600 mt-2">
            Settlement Cycle: T+2 days | Minimum: ₹1,000 | Admin approval
            required
          </Text>
        </View>
              ...
              <View className="bg-gradient-to-r from-blue-50 to-blue-100 rounded-xl p-4 mb-3">
                ...
                <View className="mb-3 pb-3 border-b border-blue-200">
                  ...
                  <View className="flex-row items-center justify-between">
                    ...
                    <TouchableOpacity
                      onPress={() => setShowBankNumber(!showBankNumber)}
                    >
                      {showBankNumber ? (
