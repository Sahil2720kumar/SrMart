...
interface DeliveryOTPVerificationModalProps {
  ...
  selectedOrderForOtp: DeliveryOrder | null;
  otpInput: string;
  setOtpInput: (otpInput: string) => void;
  handleVerifyOtp: () => void;
}

const DeliveryOTPVerificationModal = ({
  showOtpModal,
  selectedOrderForOtp,
  setOtpInput,
  otpInput,
  setShowOtpModal,
  handleVerifyOtp,
}: DeliveryOTPVerificationModalProps) => {
  return (
    <Modal
    ...
    >
      <View className="flex-1 bg-black/50 items-center justify-center p-4">
        <View className="bg-white rounded-3xl w-full max-w-md p-6">
          ...
          {selectedOrderForOtp && (
            <View className="mb-6">
              <Text className="text-sm font-semibold text-gray-700 mb-2">
                Customer: {selectedOrderForOtp.customer.name}
              </Text>
              <Text className="text-xs text-gray-500 mb-4">
                Order: {selectedOrderForOtp.order_number}
              </Text>
          ...
          <View className="flex-row gap-3">
            ...
            <TouchableOpacity
              ...
              className={`flex-1 py-3 rounded-xl ${
                otpInput.length === 4 ? 'bg-green-500' : 'bg-gray-300'
              }`}
              activeOpacity={0.8}
            >
          ...
          {selectedOrderForOtp && selectedOrderForOtp.deliveryOtp && __DEV__ && (
            <View className="mt-4 p-3 bg-yellow-50 rounded-xl border border-yellow-200">
              <Text className="text-xs text-center text-yellow-800 font-semibold">
                🔒 Dev Mode: OTP is {selectedOrderForOtp.deliveryOtp}
              </Text>
            </View>
          )}
        </View>
